  ...
  useEffect(() => {
    ...
    const render = (t: number) => {
      ...
      if (animationType === "hover") {
        ...
        yaw = tScaled * wY;
        pitch = Math.sin(tScaled * wX + phX) * 0.8;
        roll = Math.sin(tScaled * wZ + phZ) * 0.6;
        program.uniforms.uRot.value = setMat3FromEuler(
          yaw,
          pitch,
          roll,
    ...
    if (suspendWhenOffscreen) {
      ...
      startRAF();
      (container as ExtendedDivElement).__prismIO = io;
    } else {
      startRAF();
    }
    ...
    return () => {
      ...
      if (suspendWhenOffscreen) {
        const io = (container as ExtendedDivElement).__prismIO
        if (io) io.disconnect();
        delete (container as ExtendedDivElement).__prismIO;
      }
